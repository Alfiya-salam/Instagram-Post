...
class EventPostGenerator:
    ...
    def generate_event_image(self, prompt):
        try:
            event_name, theme = prompt.split(" - ")
            
            image_prompt = (
                f"A professional event promotional image representing '{theme}'. "
                f"Create a symbolic visual representation that captures the essence of {theme}. "
                f"The image should be modern, clean, and instantly recognizable for what the event is about. "
                f"Focus on relevant attractive and imagery that communicate the theme. "
                f"Make it look like a high-quality event banner."
            )
    ...
    def setup_agents(self):
        ...
        self.quote_creator = Agent(
            ...
            backstory="You create powerful, memorable quotes that inspire action and create FOMO.",
            verbose=True,
            llm=llm
        )
